...
def click_and_collect_get_service(click_and_collect_id):
    connection = open_database()
    cursor = connection.cursor()
    cursor.execute("SELECT * FROM click_and_collects WHERE click_and_collect_id = ?",
...
def click_and_collect_update_service(dishes_id, drinks_id, date_time, user_id, click_and_collect_id):
    ...
    cursor = connection.cursor()
    cursor.execute(
        "UPDATE click_and_collects SET dishes_id = ?, drinks_id = ?, date_time = ?, user_id = ?"
